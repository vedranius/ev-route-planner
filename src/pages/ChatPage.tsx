...
export default function ChatPage() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    await push(ref(db, 'chat/general'), msg);
    setNewMessage('');
  };
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-4 border-b border-[#334155] bg-[#1e293b]">
        ...
        <p className="text-sm text-[#94a3b8]">Chat with other EV drivers • Share tips, charger info, and updates</p>
      </div>
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`max-w-[75%] ${msg.userId === currentUser?.uid ? 'text-right' : ''}`}>
              ...
              {msg.imageUrl && (
                <img src={msg.imageUrl} alt="" className="mt-1 max-w-[200px] rounded-lg" />
              )}
            </div>
          </div>
        ))}
